...
class StudentEdit {
  ...
  initializeForm() {
    ...
    this.setInputValue(
      "date-of-birth",
      this.formatDateForInput(this.currentStudent.dateOfBirth)
    );
    ...
    this.setInputValue(
      "school-faculty",
      this.currentStudent.school || this.currentStudent.faculty
    );
    this.setInputValue("department", this.currentStudent.department);
    this.setInputValue(
      "course-of-study",
      this.currentStudent.courseOfStudy || this.currentStudent.program
    );
    this.setSelectValue("level", this.currentStudent.level);
    this.setInputValue(
      "matric-number",
      this.currentStudent.matricNumber || this.currentStudent.studentId
    );
  ...
  populateSkills() {
    ...
    if (this.skills.length === 0) {
      skillsContainer.innerHTML =
        '<p class="text-slate-500 dark:text-slate-400 text-sm">No skills added yet.</p>';
      return;
    }
  ...
  updateFileDisplay(inputId, file) {
    ...
    if (!fileInfo) {
      ...
      fileInfo.className =
        "file-info mt-2 p-2 bg-slate-50 dark:bg-slate-800 rounded text-xs";
  ...
  validateFile(file, inputId) {
    ...
    if (file.size > maxSizes[inputId]) {
      alert(
        `File size must be less than ${maxSizes[inputId] / 1024 / 1024}MB.`
      );
      return false;
    }
  ...
  setupDragAndDrop() {
    ...
    const coverLetterDropZone = document.getElementById(
