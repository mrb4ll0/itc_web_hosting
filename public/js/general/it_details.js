...
class InternshipDetails {
  ...
  async loadStudentImage(userId) {
    try {
      ...
      if (student) {
        this.top_right_image.src = student.imageUrl || '../images/default-profile.png';
      }
    } catch (error) {
  ...
  async loadInternshipDetails() {
    try {
      this.showLoading();
      const internship = await companyCloud.getInternshipById(this.internshipId);
  ...
  renderInternshipDetails(internship) {
    ...
    const container = document.getElementById("internship-details-container");
    if (!container) return;
     //console.log("internship "+JSON.stringify(internship));
    container.innerHTML = `
          ...
          <div class="p-8">
            <div class="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6">
              <div class="flex items-start space-x-6 flex-1">
                ...
                <div class="flex-1">
                  <h1 class="text-3xl font-bold text-gray-800 dark:text-gray-100 mb-2">${internship.title}</h1>
                  <p class="text-xl text-blue-600 dark:text-blue-400 font-semibold mb-3">${internship.company.name}</p>
                  <div class="flex flex-wrap gap-2">
                    <span class="inline-flex items-center px-3 py-1 bg-blue-100 text-blue-800 rounded-full text-sm font-medium">
                      📍 ${internship.company.address}, ${internship.company.localGovernment}, ${internship.company.state}
                    </span>
              ...
              <div class="flex flex-col gap-3">
                <button onclick="internshipDetails.applyForInternship('${internship.status}','${internship.title}','${internship.company.name}','${internship.id}')" 
                        class="px-8 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition font-semibold text-lg shadow-md">
                  Apply Now
                </button>
                <button onclick="internshipDetails.saveInternship('${internship.id}')" 
              ...
              <div class="space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div class="flex items-center text-gray-600 dark:text-gray-400">
                    ...
                    <span>Applications: <strong class="text-gray-800 dark:text-gray-200">${internship.applicationsCount}</strong></span>
                  </div>
                  <div class="flex items-center text-gray-600 dark:text-gray-400">
                    <span class="mr-3">📊</span>
                    <span>Status: <strong class="text-gray-800 dark:text-gray-200 capitalize">${internship.status}</strong></span>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-3">
                  <div class="flex justify-between items-center p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                    ...
                    <span class="text-blue-600 dark:text-blue-400 font-semibold">${this.formatDate(internship.startDate)}</span>
                  </div>
                  <div class="flex justify-between items-center p-3 bg-green-50 dark:bg-green-900/20 rounded-lg">
                    <span class="text-gray-700 dark:text-gray-300 font-medium">Duration:</span>
                    <span class="text-green-600 dark:text-green-400 font-semibold">${internship.duration}</span>
                  </div>
                </div>
                <div class="space-y-3">
                  <div class="flex justify-between items-center p-3 bg-purple-50 dark:bg-purple-900/20 rounded-lg">
                    ...
                    <span class="text-purple-600 dark:text-purple-400 font-semibold">${this.formatDate(internship.endDate)}</span>
                  </div>
                  <div class="flex justify-between items-center p-3 bg-orange-50 dark:bg-orange-900/20 rounded-lg">
                    ...
                    <span class="text-orange-600 dark:text-orange-400 font-semibold">${internship.stipendAvailable ? this.formatCurrency(internship.stipend) : 'Unpaid'}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6">
              ...
              <div class="space-y-4">
                <div class="flex items-center space-x-3">
                  ...
                  <div>
                    <h3 class="font-semibold text-gray-800 dark:text-gray-100">${internship.company.name}</h3>
                    <p class="text-sm text-gray-600 dark:text-gray-400">${internship.company.industry}</p>
                  </div>
                </div>
                ...
                <div class="space-y-2 text-sm">
                  <div class="flex items-center text-gray-600 dark:text-gray-400">
                    ...
                    <span>${internship.company.address}, ${internship.company.localGovernment}</span>
                  </div>
                  <div class="flex items-center text-gray-600 dark:text-gray-400">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6">
              ...
              <div class="space-y-2 text-sm text-gray-600 dark:text-gray-400">
                <div class="flex justify-between">
                  <span>Posted:</span>
                  <span class="font-medium">${this.formatDate(internship.postedAt)}</span>
                </div>
                <div class="flex justify-between">
                  <span>Last Updated:</span>
                  <span class="font-medium">${this.formatDate(internship.updatedAt)}</span>
                </div>
                <div class="flex justify-between">
                  <span>Position:</span>
                  <span class="font-medium capitalize">${internship.status}</span>
                </div>
              </div>
            </div>
          </div>
  ...
  async applyForInternship(status,title,company,id) {
