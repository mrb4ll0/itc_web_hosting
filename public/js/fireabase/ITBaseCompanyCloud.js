...
class ITBaseCompanyCloud {
  ...
  async getCompanyStats(companyId) {
    ...
    try {
      ...
      const industrialTrainings = await this.getCompanyIndustrialTrainings(
        companyId
      );
      ...
      for (const it of industrialTrainings) {
        ...
        const applications = await this.getApplicationsForIndustrialTraining(
          companyId,
          it.id
        );
    ...
    try {
      ...
      const applications = await Promise.all(
        snapshot.docs.map(async (docSnap) => {
          ...
          const industrialTraining = await this.getIndustrialTrainingById(
            companyId,
            itId
          );
      ...
      console.error(
        "Error getting applications for industrial training:",
        error
      );
      throw error;
    }
  ...
  async updateInternshipStatus(companyId, itId) {
    ...
    try {
      ...
      if (newStatus !== internship.status) {
        await this.updateInternshipStatusInDatabase(
          companyId,
          itId,
          newStatus,
  ...
  async updateInternshipStatusInDatabase(
    companyId,
    itId,
    status,
  ...
  async updateApplicationStatus(
    ...
    if (!companyId || !itId || !applicationId) {
      throw new Error(
        "Company ID, Industrial Training ID, and Application ID are required"
      );
    }
    ...
    try {
      ...
      switch (status) 
      {
         ...
         case 'rejected':
             ...
             break;
          case 'pending':
             statuss = 'Under Review';
             break;
      }
      await sendWebNotification(
        student.fmcToken,
        company.name,
        "Your " + application.internship.title + " application is " + statuss
      );
    } catch (error) {
  ...
  async getActivePostings(companyId) {
    ...
    try {
      const allIndustrialTrainings = await this.getCompanyIndustrialTrainings(
        companyId
      );

      // Filter active postings
      const activePostings = allIndustrialTrainings.filter(
        (it) => it.status === "active" || it.status === "open"
      );
  ...
  async searchIndustrialTrainings(companyId, searchTerm) {
    ...
    try {
      ...
      return allIndustrialTrainings.filter(
        (it) =>
          (it.title && it.title.toLowerCase().includes(lowercaseSearch)) ||
          (it.department &&
            it.department.toLowerCase().includes(lowercaseSearch)) ||
          (it.description &&
            it.description.toLowerCase().includes(lowercaseSearch))
      );
    } catch (error) {
  ...
  async removeImageFromGallery(companyId, imageUrl) {
    ...
    try {
      ...
      const currentGallery = Array.isArray(data.galleryImages)
        ? data.galleryImages
        : [];
  ...
  async setITStatus(companyId, itId, status) {
    if (!companyId || !itId || !status) {
      throw new Error(
        "Company ID, Industrial Training ID, and Status are required"
      );
    }
  ...
  async removeValueFromCompanyArray(companyId, arrayFieldName, valueToRemove) {
    ...
    try {
      ...
      const data = companySnap.data();
      const currentArray = Array.isArray(data[arrayFieldName])
        ? data[arrayFieldName]
        : [];
   ...
   * @returns {Promise<void>}
   */
  async updateValueInCompanyArray(
    companyId,
    arrayFieldName,
    oldValue,
    newValue
  ...
  async removeFileFromIT(companyId, itId, urlToRemove) {
    ...
    try {
      ...
      const data = itDoc.data();
      const currentFiles = Array.isArray(data.attachedFiles)
        ? data.attachedFiles
        : [];
  ...
  async getApplicationById(companyId, itId, applicationId) {
    ...
    try {
      ...
      if (applicationDoc.exists()) {
        ...
        data.student = await new StudentCloudDB().getStudentById(
          data.student.uid
        );
        ...
        industrialTraining.company = await this.getCompany(
          industrialTraining.company.id
        );
        studentApp.student = await studentCloudDB.getStudentById(
          studentApp.student.uid
        );
  ...
  async updateCompanyApplicationDuration(
    durationObject,
    applicationId,
    itId,
    sid
    ...
    if (!companyId || !applicationId || !durationObject) {
      throw new Error(
        "Company ID, Application ID, and duration object are required"
      );
    }
  ...
  /**
 * Get total count of accepted applications for a specific industrial training
 * @param {string} companyId - The company ID
 * @param {string} itId - The industrial training ID
 * @returns {Promise<number>} Count of accepted applications
 */
async getTotalAcceptedApplications(companyId, itId) {
  if (!companyId || !itId) {
    throw new Error("Company ID and Industrial Training ID are required");
  }
  ...
  try {
    ...
    // Create a query to get only accepted applications
    const q = query(
      applicationsRef,
    ...
    console.error("Error getting total accepted applications:", error);
    throw error;
  }
}
...
/**
 * Get application counts by status for a specific industrial training
 * @param {string} companyId - The company ID
 * @param {string} itId - The industrial training ID
 * @param {string[]} statuses - Array of statuses to count (optional, defaults to ['accepted'])
 * @returns {Promise<Object>} Object with status counts
 */
async getApplicationCountsByStatus(companyId, itId, statuses = ['accepted']) {
  if (!companyId || !itId) {
    throw new Error("Company ID and Industrial Training ID are required");
  }
  ...
  try {
    ...
    // Optionally, get total count as well
    const totalQuery = query(applicationsRef);
    const totalSnapshot = await getDocs(totalQuery);
    counts.total = totalSnapshot.size;

    return counts;
  } catch (error) {
    console.error("Error getting application counts by status:", error);
    throw error;
  }
}
