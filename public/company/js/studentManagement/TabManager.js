...
class TabManager {
    ...
    async initializeDashboard() {
        try {
            ...
            this.applications = await it_base_companycloud.getAllCompanyApplications(this.companyId);
    ...
    async processApplicationsData() {
        ...
        this.applicationsByStatus = {
            ...
            shortlisted: this.applications.filter(app => 
    ...
    calculateApplicationStats() {
        ...
        this.applicationStats = {
            ...
            shortlistedRate: total > 0 ? ((shortlistedCount / total) * 100).toFixed(1) : 0,
            acceptanceRate: total > 0 ? ((acceptedCount / total) * 100).toFixed(1) : 0,
        };
    ...
    initializeSharedElements() {
        ...
        // Stats elements
        this.totalApplicantsValue = document.getElementById("total-applicants-value");
    ...
    setupTabEvents() {
        const tabs = document.querySelectorAll('button[class*="px-6 py-3 rounded-lg"]');
    ...
    try {
        ...
        const htmlContent = await this.fetchHTML(`../company/studentManagement/${tabId}.html`);
    ...
    async initializeTabJS(tabId) {
        ...
        try {
            ...
            console.log(error);
            console.warn(`No JavaScript module found for ${tabId}, continuing without it.`);
        }
    }
    ...
    handleLoadError(error) {
        ...
        alert(`Error loading data: ${errorMessage}. Please try refreshing the page.`);
