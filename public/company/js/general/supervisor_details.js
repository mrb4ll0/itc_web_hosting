...
class SupervisorManagementController {
    ...
    setupEventListeners() {
        ...
        document.getElementById('activateSupervisorBtn').addEventListener('click', () => this.activateSupervisor());
        document.getElementById('deactivateSupervisorBtn').addEventListener('click', () => this.deactivateSupervisor());

        // Student assignment
        document.getElementById('assignStudentsBtn').addEventListener('click', () => this.showAssignModal());
        document.getElementById('searchStudents').addEventListener('input', (e) => this.filterAvailableStudents(e.target.value));

        // Modal controls
        document.getElementById('closeModalBtn').addEventListener('click', () => this.hideAssignModal());
        document.getElementById('cancelAssignBtn').addEventListener('click', () => this.hideAssignModal());
        document.getElementById('confirmAssignBtn').addEventListener('click', () => this.assignSelectedStudents());
    ...
    async loadSupervisorData() {
        try {
            ...
            this.currentSupervisor = await this.managementService.getSupervisorById(this.supervisorId);
    ...
    updateSupervisorUI() {
        ...
        // Basic info
        document.getElementById('supervisorName').textContent = supervisor.displayName || 'Supervisor';
        ...
        if (supervisor.allowed) {
            statusBadge.className = 'inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300';
            statusBadge.textContent = 'Active';
            document.getElementById('activateSupervisorBtn').classList.add('hidden');
            document.getElementById('deactivateSupervisorBtn').classList.remove('hidden');
        } else {
            statusBadge.className = 'inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-amber-100 text-amber-800 dark:bg-amber-900 dark:text-amber-300';
            statusBadge.textContent = 'Pending Activation';
            document.getElementById('activateSupervisorBtn').classList.remove('hidden');
            document.getElementById('deactivateSupervisorBtn').classList.add('hidden');
        }
    ...
    async loadStudentsData() {
        try {
            ...
            this.assignedStudents = await this.managementService.getAssignedStudents(this.supervisorId);
            ...
            this.availableStudents = await this.managementService.getAvailableStudents();
    ...
    renderAssignedStudents() {
        ...
        if (!this.assignedStudents || this.assignedStudents.length === 0) {
            ...
            // Update empty state message based on supervisor status
            const emptyMessage = emptyState.querySelector('p');
            if (this.currentSupervisor && !this.currentSupervisor.allowed) {
                emptyMessage.textContent = 'Activate supervisor account to assign students.';
            } else {
                emptyMessage.textContent = 'Assign students to this supervisor to get started.';
            }
            return;
        }
        ...
        // Safely render students
        this.assignedStudents.forEach(student => {
            if (student && student.uid) { // Check if student object is valid
...
renderAvailableStudents() {
    ...
    if (this.currentSupervisor && this.currentSupervisor.students) {
        this.currentSupervisor.students.forEach(uid => assignedStudentUids.add(uid));
    }
    ...
    if (availableAndUniqueStudents.length === 0) {
        ...
        const emptyMessage = emptyState.querySelector('p');
        if (this.assignedStudents && this.assignedStudents.length > 0) {
            emptyMessage.textContent = 'All available students are already assigned to this supervisor.';
        } else {
...
getUniqueStudents(students) {
    ...
    async assignStudentToSupervisor(student) {
        try {
            ...
            const success = await this.managementService.assignStudentToSupervisor(this.supervisorId, student.uid);
    ...
    async removeStudentFromSupervisor(student) {
        try {
            ...
            const studentName = student?.name || 'Student';
            const confirmed = confirm(`Are you sure you want to remove ${studentName} from this supervisor?`);
            
            if (!confirmed) return;

            const success = await this.managementService.removeStudentFromSupervisor(this.supervisorId, student.uid);
    ...
    async activateSupervisor() {
        try {
            const success = await this.managementService.activateSupervisor(this.supervisorId);
    ...
    async deactivateSupervisor() {
        try {
            const confirmed = confirm('Are you sure you want to deactivate this supervisor?');
            
            if (!confirmed) return;

            const success = await this.managementService.deactivateSupervisor(this.supervisorId);
    ...
    const availableAndUniqueStudents = this.getUniqueStudents(this.availableStudents)
        .filter(student => student && student.uid && !assignedStudentUids.has(student.uid));
    ...
    createModalStudentElement(student) {
        ...
        div.innerHTML = `
            <div class="flex items-center space-x-3">
                <input type="checkbox" id="student-${studentId}" class="student-checkbox h-4 w-4 text-blue-500 focus:ring-blue-500 border-slate-300 dark:border-slate-600 rounded" ${isSelected ? 'checked' : ''}>
    ...
    updateSelectedCount() {
        document.getElementById('selectedCount').textContent = `${this.selectedStudents.size} students selected`;
    }
    ...
    async assignSelectedStudents() {
        ...
        try {
            const success = await this.managementService.assignMultipleStudentsToSupervisor(
                this.supervisorId, 
                Array.from(this.selectedStudents)
            );
            
            if (success) {
                this.showSuccess(`${this.selectedStudents.size} students assigned successfully`);
    ...
    filterAvailableStudents(searchTerm) {
        ...
        const filteredStudents = this.availableStudents.filter(student => {
            ...
            const companyMatch = student.company?.toLowerCase().includes(searchTerm.toLowerCase()) || false;
